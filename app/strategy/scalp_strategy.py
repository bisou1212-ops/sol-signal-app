...
핵심 변경점 (v4): 거래량스파이크를 "5개 중 하나"에서 "필수 게이트"로 승격했다.
백테스트 31건 분석 결과 거래량스파이크 충족 시 승률 66.7% vs 미충족 시 36.4%로 격차가
뚜렷해서, RSI 크로스가 나도 거래량스파이크가 없으면 진입하지 않도록 안전장치를 추가.
(화면 표시는 영향 없음 - 5개 조건 점수/근거는 여전히 항상 다 보여줌)
...
4. 거래량스파이크: 최근 거래량이 20봉 평균 대비 배수 이상. **필수 게이트** (미충족 시 진입 불가). progress = 배수/기준배수
...
class ScalpComposite:
    total_score: float
    direction: Direction           # 실제 진입 가능 방향 (RSI 크로스 + 거래량스파이크 둘 다 충족 시에만 LONG/SHORT)
    agreement_count: int
    total_strategies: int
    breakdown: list[dict] = field(default_factory=list)
    lean: str = "neutral"          # 화면 표시용 리닝 방향 (추세 기준, 크로스 전이어도 표시)
    status: str = "ok"             # ok | warmup_trend | warmup_main | rsi_na | flat_trend
    crossed: bool = False          # RSI 크로스 자체는 일어났는지 (거래량 게이트와 별개로 표시용)
...
def evaluate_scalp(main_df: pd.DataFrame, trend_df: pd.DataFrame) -> ScalpComposite:
    ...
    # 실제 진입 가능 방향: RSI 실제 크로스 + 거래량스파이크 둘 다 충족된 경우만
    # (백테스트 31건 분석: 거래량스파이크 충족 시 승률 66.7% vs 미충족 36.4% -> 필수 게이트로 승격)
    trigger_direction = lean if (rsi_crossed and volume_spike) else Direction.NEUTRAL

    return ScalpComposite(total_score, trigger_direction, met, len(conditions), breakdown, lean=lean.value, crossed=rsi_crossed)
